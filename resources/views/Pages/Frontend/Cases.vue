        ...
        <main>
            ...
            <section class="section">
                <div class="container case-grid">
                    <article v-for="(item, idx) in items" :key="idx" class="case-card">
                        <div class="case-tag">{{ item.industry }}</div>
                        <h3>{{ item.name }}</h3>
                        <p>{{ item.summary }}</p>
                    </article>
                </div>
...
.case-card { background: #fff; border: 1px solid $line; border-radius: 20px; padding: 26px 22px; box-shadow: 0 12px 30px rgba(15, 23, 42, 0.03); }
.case-tag { display: inline-flex; margin-bottom: 12px; background: #eef2ff; color: #4f46e5; padding: 7px 10px; border-radius: 999px; font-size: 12px; }
.case-card h3 { margin: 0 0 12px; font-size: 24px; }
.case-card p { margin: 0; color: $muted; line-height: 1.9; }
...
@media (max-width: 980px) { .nav { display: none; } .case-grid { grid-template-columns: 1fr; } }
