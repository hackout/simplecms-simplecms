<template>
    ...
    <VTable action="backend.user_log.list" :params="query" :round="false" :padding="false" @search="searchData" ref="tableRef">
        <template #header_left>
            <el-button :loading="cleaning" type="danger" @click="clean">清空记录</el-button>
        </template>
        <template #header_right>
            <el-form-item>
                <el-input v-model="query.keyword" clearable @clear="searchData" placeholder="关键词"></el-input>
            </el-form-item>
            <el-form-item>
...
export default {
    ...
    methods: {
        ...
        async deleteItem(item, refName) {
            let res = await this.$axios.delete(this.$route('backend.user_log.delete', { id: item.id }))
            if (res.code == this.$config.successCode) {
                this.$message.success("删除记录信息成功");
        ...
        async clean() {
            if (!this.cleaning) {
                this.cleaning = true
                let res = await this.$axios.post(this.$route('backend.user_log.clean', { id: this.item.id }))
                this.cleaning = false
                if (res.code == this.$config.successCode)
